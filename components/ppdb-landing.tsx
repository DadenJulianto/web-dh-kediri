...
const programs = [
  { icon: BookOpen, title: 'Tahfizh ', text: 'Hafalan terarah dengan talaqqi, tasmi’ dan pendampingan ustadz yang amanah.' },
  { icon: GraduationCap, title: 'Bahasa ', text: 'Menyelenggarakan pembelajaran bahasa yang menarik dan efektif.' },
  { icon: Landmark, title: 'Talent', text: 'Mengembangkan bakat dan minat santri dalam berbagai bidang.' },
  
]

const activities = [
  ['01', 'Bela Diri Ju Jitsu', 'Membangun keberanian dan ketangguhan diri.'],
  ['02', 'Kepanduan', 'Belajar memimpin, bekerja sama dan mencintai alam.'],
  ['03', 'Sepak Bola', 'Menjaga kebugaran melalui olahraga tim yang seru.'],
...
const testimonials = [
  ['/indra.jpg', '“Terima kasih para asatidz yang dengan sabar membimbing para santri yang sedang dalam masa mencari jati diri.”', 'Indra', 'Wali Santri'],
  ['/e-tanjung.webp', '“Respons ustadz bagus, penyampaian informasi dari pondok baik, dan ramah dalam menjawab setiap pertanyaan tentang pondok maupun anak santri.”', 'E-Tanjung', 'Wali Santri'],
  ['/sri-rahayu.jpg', '“Pondok yang nyaman untuk anak, memperhatikan kebutuhan santri dan kondisi wali santri, serta selalu berusaha memberikan yang terbaik.”', 'Sri Rahayu', 'Wali Santri'],
]
...
export function PpdbLanding() {
  ...
  const nav = [['#visi', 'Visi & Misi'], ['#program', 'Program'], ['#ekstrakurikuler', 'Ekstrakurikuler'], ['#testimoni', 'Kata Mereka'], ['#pendaftaran', 'Pendaftaran'], ['#syarat', 'Syarat'], ['#kontak', 'Kontak']]
  return <div className="min-h-screen bg-cream text-ink">
    ...
    <main>
      ...
      <section className="bg-cream px-5 py-24 lg:px-8"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="06 — Galeri Kegiatan" title="Hari-hari yang penuh cerita." text="Potret kecil dari kegiatan belajar, pembinaan, dan kebersamaan di Darul Hijrah Kediri." /><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{gallery.map(([img, caption]) => <figure key={caption} className="group relative aspect-[4/5] overflow-hidden rounded-[24px] border border-emerald-900/10 bg-sand shadow-[0_12px_24px_rgba(11,46,41,0.06)]"><img src={img} alt={caption} className="size-full object-cover transition duration-500 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/15 to-transparent" /><figcaption className="absolute inset-x-0 bottom-0 px-5 pb-5 pt-14 text-sm font-semibold text-white">{caption}</figcaption></figure>)}</div></div></section>
